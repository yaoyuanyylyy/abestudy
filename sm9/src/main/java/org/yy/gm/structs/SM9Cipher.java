...
public class SM9Cipher {
    ...
    public SM9Cipher(EnType enType, Element C1, byte[] C2, byte[] C3) {
        this.enType = enType;
        this.C1 = C1;
        this.C2 = C2;
        this.C3 = C3;
    ...
    public static SM9Cipher fromByteArray(SM9KeyParameters keyParameters, byte[] in, int inOff, int inLen) {
        inLen += inOff;
        EnType enType = EnType.values()[in[0]];
        inOff += 1;
        Element C1 = keyParameters.parameters.pairing.getG1().newElementFromBytes(
                Arrays.copyOfRange(in, inOff, inOff+ SM9CurveParameters.LEN_G1_ELEMENT));
        inOff += SM9CurveParameters.LEN_G1_ELEMENT;
        int digestLen = CipherUtils.getDigestLength(SM9Config.algDigest);
