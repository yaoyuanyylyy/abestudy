 ...
 */
public class SM9EncryptEngineTest extends SM9KEMEngineTest {
    @Override
    ...
    private SM9Cipher encrypt(byte[] in, int inOff, int inLen) throws InvalidCipherTextException {
        ...
        if(isBasedBlockCipher)
            SM9LogUtils.showMsg("加密明文的方法为分组密码算法:");
        else
            SM9LogUtils.showMsg("加密明文的方法为基于KDF的序列密码:");
        SM9LogUtils.showMsg("klen: "+(k1Len+keyBytes) + " bytes, 0x"+(Integer.toHexString((k1Len+keyBytes)*8)));
        ...
        if (isBasedBlockCipher) {
            ...
            try {
                ...
                throw new InvalidCipherTextException(SM9Config.algSymm.name() + " encrypt error.", e);
            }
        } else {
            //C2=M^K1
        ...
        SM9LogUtils.showMsg("K1:");
        SM9LogUtils.showMsg(SM9LogUtils.toHexString(K1));
        SM9LogUtils.showMsg("C2:");
        SM9LogUtils.showMsg(SM9LogUtils.toHexString(C2));
        SM9LogUtils.showMsg("K2:");
        SM9LogUtils.showMsg(SM9LogUtils.toHexString(K2));
        SM9LogUtils.showMsg("C3:");
        SM9LogUtils.showMsg(SM9LogUtils.toHexString(C3));
    ...
    private byte[] decrypt(byte[] in, int inOff, int inLen) throws InvalidCipherTextException {
        ...
        if (isBasedBlockCipher) {
            ...
            try {
                ...
                throw new InvalidCipherTextException(SM9Config.algSymm.name() + " decrypt error.", e);
            }
        } else {
            // M=C2^K1
        ...
        SM9LogUtils.showMsg("K1':");
        SM9LogUtils.showMsg(SM9LogUtils.toHexString(K1));
        SM9LogUtils.showMsg("M':");
        SM9LogUtils.showMsg(SM9LogUtils.toHexString(M));
        ...
        SM9LogUtils.showMsg("K2':");
        SM9LogUtils.showMsg(SM9LogUtils.toHexString(K2));
        SM9LogUtils.showMsg("u:");
        SM9LogUtils.showMsg(SM9LogUtils.toHexString(u));

        if (!SM9Utils.byteEqual(u, cipherText.C3))
            throw new InvalidCipherTextException("C3 verify failed");

        SM9LogUtils.showMsg("u=C3'");
