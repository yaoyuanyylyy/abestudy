...
public final class SM9Utils {
    ...
    public static MessageDigest createDigest() {
        try {
            return new BCJcaJceHelper().createMessageDigest("SM3");
     ...
     *
     * @param publicKey 签名或加密主公钥
     * @param hid             私钥生成函数识别符
     * @param id              用户标识
     * @return 标准中的返回描述为签名主公钥或加密主公钥对象，这里直接返回 Element，以便在代码中使用。
     */
    public static Element preQ(SM9PublicKey publicKey, byte hid, String id) {
        // Q =[H1(ID||hid, N)]P +Ppub
        BigInteger h1 = H1(id, hid, publicKey.parameters.N);
        return publicKey.parameters.P1.mul(h1).add(publicKey.Q).getImmutable();
    }
